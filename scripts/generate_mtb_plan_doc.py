...
PAGE_ROWS = [
    ...
    ("ReferralsPage", "Социальный сценарий, приглашения и работа с реферальной механикой."),
    ("LeaderboardScreen", "Турнирная таблица игроков."),
...
PLAN_ROWS = [
    ...
    ("Социальный модуль / друзья", "Выполнено в расширенном виде", "Реализованы приглашения, лидерборд и социальное кольцо вместо простого списка друзей."),
    ...
    ("AI/QR экраны из исходного плана", "Адаптировано", "В текущем MVP упор смещен на игровые, квестовые и административные сценарии; отдельные AI и QR экраны не вынесены в финальный web-интерфейс."),
]
...
def build_document() -> Path:
    ...
    add_paragraph(
        doc,
        "Ниже приведено сопоставление основных пунктов исходного Word-плана и фактической реализации в репозитории. Это позволяет показать, какие элементы полностью сохранены, какие были усилены и какие были адаптированы под формат web-MVP.",
    )
    ...
    add_paragraph(
        doc,
        f"OpenAPI-описание проекта содержит {sum(len(value) for value in OPENAPI.get('paths', {}).values())} основных endpoint-ов. Они покрывают авторизацию демо-пользователя, профиль галактики, события, квесты, награды, мини-игры, рефералов, лидерборд и административный функционал.",
    )
